...
#
# @param repo_config
#   Specifies a Hash of server-side repo configuration options. These options are place in a YAML configuration file that Atlantis reads
#   on startup.
#
# @param environment
#   Specifices an Array of `KEY=VALUE` pairs. These environment variables are passed to Atlantis on startup and are useful for modifying
#   Atlantis behavior and setting environment variables for use in Terraform code at a global level.
#
# @param user
...
#
# @param proxy
#   Specifies a proxy server to use for external network access if required. If set, this value is also used to inject `*_PROXY` 
#   environment variables into the Atlantis process environment.
#
# @param no_proxy
...
class atlantis (
  ...
  $_default_config = {
    'atlantis-url' => "http://${$facts['networking']['fqdn']}:4141",
    'port'         => 4141,
  }
